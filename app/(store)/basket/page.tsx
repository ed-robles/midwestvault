...
function BasketPage() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      console.error("Error creating checkout sesson:", error);
    } finally {
      setIsLoading(false);
    }
  };
              ...
              >
                <div className="w-20 h-20 sm:h-24 flex-shrink-0 mr-4">
                  {item.product.gallery && (
                    <Image
                      src={imageUrl(item.product.gallery).url()}
        ...
        <div className="w-full lg:w-80 lg:sticky lg:top-4 h-fit bg-white p-6 border rounded order-first lg:order-last fixed bottom-0 left-0 lg:left-auto">
          ...
          {isSignedIn ? (
            ...
            <SignInButton mode="modal">
              <button className="mt-4 w-full bg-gray-100 border px-4 py-2 rounded hover:bg-gray-200">
                Log In to Checkout
              </button>
            </SignInButton>
          )}
